...
Promise.all([d3.json(topoJsonPath), d3.csv(csvPath)])
  .then(function ([topoJsonData, csvData]) {
    ...
    const selectedState = topojson.feature(topoJsonData, topoJsonData.objects.states).features.find(feature => feature.properties.name === stateAbbreviations[feature.properties.name]);
